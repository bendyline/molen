...
- **Path:** `content/earth/businesses/catalog.json`
- **Local notice:** [`content/earth/businesses/README.md`](content/earth/businesses/README.md)
...
- **Paths:** `content/earth/businesses/catalog.json`,
...
This also covers the real aircraft types depicted in `content/entities/source/aircraft/`
(P-51D Mustang; OH-6), identified by their public designations. Their models are original low-poly
interpretations generated by committed scripts (`models/generate.mjs` in each source bundle) from
three.js primitives and committed parameters; no third-party mesh, texture or bitmap is used. The
`molen.entities` content pack carries the same statement in its own `NOTICE.md`.
...
```
examples/home/public/fonts/hanken-grotesk-400.woff2
examples/home/public/fonts/hanken-grotesk-600.woff2
examples/home/public/fonts/pt-serif-400.woff2
```

The samples home page (`examples/home`) uses two third-party typefaces, redistributed under the
SIL Open Font License 1.1. Each license, with its copyright line and Reserved Font Names, is
beside the files:

| Typeface | Copyright | License file |
| --- | --- | --- |
| Hanken Grotesk | © 2020 Alfredo Marco Pradil, Reserved Font Name "Hanken Grotesk" | `LICENSE-HankenGrotesk-OFL-1.1.txt` |
| PT Serif | © 2010 ParaType Ltd., Reserved Font Names "PT Sans", "PT Serif" and "ParaType" | `LICENSE-PTSerif-OFL-1.1.txt` |

The fonts are not in any npm package. OFL permits redistribution with the license; it does not
permit selling the fonts by themselves, and a modified version may not use a Reserved Font Name.
...
- `content/entities/assets/**` (trees, boulder, aircraft) and
...
- Golden and reference images under `**/test/golden/**`, and the preview images beside the
  examples and package READMEs, are Molen's own renders.
- The content packs built from `content/` carry their notices inside the pack:
  `content/entities/NOTICE.md`, `content/worldgen/NOTICE.md`, `content/sky/NOTICE.md` (the Bright
  Star Catalogue credit) and `content/earth/businesses/README.md` (§3 and §4).
- No third-party icon set or audio file is redistributed; the only third-party fonts are those in §6.
...
WorldCover vintage in §1 and the image model version in §5.
