...
export class WorldVehicles {
  readonly world: World = new World({ tickRate: 60, seed: 'world-vehicles' });
  ...
  private canOccupy(
    ...
    if (this.intersectsAircraft(box)) return false;
  ...
  private canExit(pos: [number, number, number], vehicle: string): boolean {
    ...
    if (this.intersectsAircraft(person, vehicle)) return false;
  ...
  private intersectsAircraft(box: OBB, except?: string): boolean {
