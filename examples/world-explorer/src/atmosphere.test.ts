...
import { createExplorerFog, createExplorerSky, updateExplorerFog } from './atmosphere';
import { explorerPerformanceTier } from './performance-policy';

describe('explorer horizon visibility', () => {
  it('keeps neighborhoods clear and reserves stronger haze for the distance across device tiers', () => {
    const fog = createExplorerFog('webgl');
    const visibility: number[] = [];
    for (let level = 0; level <= 5; level++) {
      updateExplorerFog(fog, explorerPerformanceTier(level).terrain.viewDistance, 150);
  ...
  it('preserves the foreground at the default aerial viewpoint on both backends', () => {
    for (const backend of ['webgl', 'webgpu'] as const) {
      const fog = createExplorerFog(backend);
      updateExplorerFog(fog, 60_000, 1_400);
      expect(MathUtils.smoothstep(2_000, fog.near, fog.far)).toBe(0);
  ...
  it('keeps the ground visible from high altitude and fades before short fixed-grid edges', () => {
    const fog = createExplorerFog('webgpu');
    updateExplorerFog(fog, 110_000, 17_000);
    expect(MathUtils.smoothstep(17_000, fog.near, fog.far)).toBeLessThan(0.15);
    updateExplorerFog(fog, 500, 150);
...
describe('explorer atmosphere backends', () => {
  it('uses native node shaders on WebGPU and preserves the legacy clear-sky parameters', async () => {
    const legacy = await createExplorerSky('webgl');
    const nodes = await createExplorerSky('webgpu');
    try {
