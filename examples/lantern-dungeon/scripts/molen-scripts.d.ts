...
// Ambient types for the scripts in this directory, from "scene.json".
...
/** Dungeon health, inventory and objective state. */
type MolenDungeonGameData = {
  status: string;
  health: number;
  key: boolean;
  doorOpen: boolean;
...
/** Enemy spawn and combat state. */
type MolenSentinelData = {
...
interface MolenComponentData {
  ...
  /** Dungeon health, inventory and objective state. */
  'dungeonGame': MolenDungeonGameData;
  /** Singleton scene environment: optional Earth/custom sky, ambient/sun lighting, background, fog, tone mapping, shadow quality tier. */
...
interface MolenCommands {
  /** Strike the first wall/enemy along your gaze; cooldown 0.4s. */
  'attack': Record<string, never>;
  /** Drink your one healing potion. */
  'heal': Record<string, never>;
  /** Open the gate with the key, or claim the lantern. */
  'interact': Record<string, never>;
  /** Relative yaw in radians, supplied by pointer movement. */
  'look': {
    yaw: number;
  };
  /** Strafe and walk [strafe, backward]. */
  'move': {
    dir: [number, number];
  };
  /** Reset the dungeon and inventory. */
  'restart': Record<string, never>;
  /** Keyboard turning; x axis controls yaw. */
  'turn': {
    dir: [number, number];
  };
}
...
interface MolenScriptConfig {
  /** script 'dungeon' */
  readonly exit: [number, number, number];
  /** script 'dungeon' */
  readonly gate: [number, number, number];
  /** script 'dungeon' */
