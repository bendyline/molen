...
/** Patrolling creature; stomp from above. */
type MolenPatrolData = {
  left: number;
  right: number;
  direction: number;
  alive: boolean;
...
/** Collectible seed state. */
type MolenSeedPickupData = {
  collected: boolean;
};
...
/** Platform run, lives and checkpoint. */
type MolenSkyGameData = {
  status: string;
  lives: number;
  coins: number;
  total: number;
...
interface MolenComponentData {
  ...
  /** Collectible seed state. */
  'seedPickup': MolenSeedPickupData;
  /** One-shot velocity set; consumed the tick it is applied. */
  'setVelocity': MolenSetVelocityData;
  /** Platform run, lives and checkpoint. */
  'skyGame': MolenSkyGameData;
  /** Named tag for selecting/grouping entities (e.g. tag.name === "enemy"). */
  'tag': MolenTagData;
  /** Snapshot-safe countdown timers that emit events (one-shot or repeating). Replaces callbacks. */
...
interface MolenCommands {
  /** Request a jump and set held state. */
  'jump': {
    held: boolean;
  };
  /** Horizontal movement; y ignored. */
  'move': {
    dir: [number, number];
  };
  /** Restart the whole level with three lives. */
...
interface MolenScriptConfig {
  /** script 'adventure' */
  readonly checkpoint: [number, number, number];
  /** script 'adventure' */
