...
it('plays the built Worker game under a hosting subpath and restarts cleanly', async () => {
  ...
  try {
    ...
    await page.waitForFunction(() => Number(document.querySelector('canvas')?.dataset.z) < 10);
    await page.keyboard.down('KeyD');
    await page.waitForFunction(
      () => Math.abs(Number(document.querySelector('canvas')?.dataset.yaw)) > 0.2,
    );
