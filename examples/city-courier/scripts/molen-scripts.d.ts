...
/** Courier run state, mirrored to the HUD. */
type MolenCourierGameData = {
  status: string;
  health: number;
  delivered: number;
  total: number;
...
interface MolenComponentData {
  ...
  /** Courier run state, mirrored to the HUD. */
  'courierGame': MolenCourierGameData;
  /** Singleton scene environment: optional Earth/custom sky, ambient/sun lighting, background, fog, tone mapping, shadow quality tier. */
...
interface MolenCommands {
  ...
  /** Steer [-1..1], throttle [-1 forward..1 reverse]. */
  'drive': {
    dir: [number, number];
  };
  /** Restart the full courier run. */
