 ...
 * disappear on the next update, and instance transforms are expanded only inside the
 * neighborhood. Meshes opt out with `userData.walkIgnore`, and water materials
 * (`userData.terrainWaterMaterial`) and `semantic:landcover` overlays are never solid.
 */
export class WalkCollision {
  ...
  constructor(private readonly ignoreVehicles: boolean = false) {}
