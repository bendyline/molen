...
} from '../src/navigation/camera-math';

const NORTH_YAW = -Math.PI / 2;

describe('navigation camera math', () => {
  ...
  it('refreshes terrain immediately when movement or panning outruns the idle cadence', () => {
    ...
    expect(
      viewNeedsImmediateUpdate(position, direction, [116, 400, 200], direction, 16, 0.04),
    ).toBe(true);
    expect(
      viewNeedsImmediateUpdate(
        position,
  ...
  it('recovers yaw and pitch from a view direction', () => {
    for (const [yaw, pitch] of [
      [NORTH_YAW, 0],
      [0.4, -0.3],
      [-2.5, 1.1],
    ] as const) {
      const recovered = orientationFromDirection(cameraForward(yaw, pitch));
      expect(recovered.yaw).toBeCloseTo(yaw, 9);
      expect(recovered.pitch).toBeCloseTo(pitch, 9);
    }
  });
});
